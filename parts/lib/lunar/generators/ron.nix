lib:
# the following is taken from https://github.com/HeitorAugustoLN/cosmic-manager/blob/7f6ff0db219457c0861afe2abb36d855a918123c/lib/ron.nix
# and is therefore available under the MIT License.
let
  inherit
    (lib)
    assertMsg
  ...
  mkThrow = scope: error: let
    prefix = messagePrefix scope;
  in
    throw "${prefix} ${trim error}";

  mkAssertion = scope: assertion: message: let
    prefix = messagePrefix scope;
  in
    assertMsg assertion "${prefix} ${trim message}";

  mkWarning = scope: message: value: let
    prefix = messagePrefix scope;
  in
  ...
  fromRON = let
    fromRON' = str: let
      ...
      splitItems = str: let
        ...
        split = acc: current: depth: rest:
          if rest == ""
          then
            if current != ""
            then acc ++ [(trim current)]
            else acc
          else let
          ...
          in
            if char == "(" || char == "[" || char == "{"
            then split acc (current + char) (depth + 1) remaining
            else if char == ")" || char == "]" || char == "}"
            then split acc (current + char) (depth - 1) remaining
            else if char == "," && depth == 0
            then split (acc ++ [(trim current)]) "" depth remaining
            else split acc (current + char) depth remaining;
      in
        split [] "" 0 content;

      findFirstColon = str: let
        helper = pos: depth:
          ...
          in
            if char == "(" || char == "[" || char == "{"
            then helper (pos + 1) (depth + 1)
            else if char == ")" || char == "]" || char == "}"
            then helper (pos + 1) (depth - 1)
            else if char == ":" && depth == 0
            then pos
            else helper (pos + 1) depth;
      in
        helper 0 0;
    ...
    in
      ...
      then map fromRON' (splitItems trimmed)
      ...
      else if match "Some\\(.*\\)" trimmed != null
      then let
      ...
      else if match "[A-Za-z_][A-Za-z0-9_]*\\(.*\\)" trimmed != null
      then let
      ...
      else if match "-?[0-9]+[.][0-9]+" trimmed != null
      then let
      ...
      else if match "-?[0-9]+" trimmed != null
      then toInt trimmed
      else if match "'.'" trimmed != null
      then {
        __type = "char";
        value = substring 1 1 trimmed;
      }
      else if match ''".*"'' trimmed != null
      then fromJSON trimmed
      else {
        __type = "raw";
        value = trimmed;
      };
  ...
  mkRON = type: value:
    {
      ...
      namedStruct = assert mkAssertion "mkRON" (
        isAttrs value
        && attrNames value
        == [
          "name"
    ...
    .${type}
    or (mkThrow "mkRON" "${type} is not supported.");

  toRON = let
    toRON' = startIndent: value: let
      type = typeOf value;
    ...
    in
      {
        ...
        float = let
          trimFloatString = float: let
            string = floatToString float;
          in
            if hasInfix "." string
            then head (match "([0-9]+[.][0-9]*[1-9]|[0-9]+[.]0)0*" string)
            else string;
        in
        ...
        in
          ...
          then "[]"
            ...
            concatImapStringsSep "\n" (
              index: element: "${indent nextIndent}${toRON' nextIndent element}${optionalString (index != count) ","}"
            )
            value
          },\n${indent startIndent}]";
        null = mkThrow "toRON" ''
        ...
        set =
          ...
          then
            ...
            then
              ...
              assert mkAssertion "toRON" (isString value.value) "raw type value must be a string.";
                value.value
            else if value.__type == "optional"
            then
              assert mkAssertion "toRON" (value ? value) "optional type must have a value.";
                if value.value == null
                then "None"
                else "Some(${toRON' startIndent value.value})"
            else if value.__type == "char"
            then
              ...
              ) "char type value must be a single character string."; "'${value.value}'"
            ...
            then
              ...
              assert mkAssertion "toRON" (isString value.variant) "enum type variant must be a string value.";
                if value ? value
                then
                  assert mkAssertion "toRON" (isList value.value) "enum type must have a list of values."; let
            ...
            then
              ...
              "map type value must be a list of attribute sets."; let
                count = length value.value;
              in
                if count == 0
                then "{}"
                  ...
                  concatImapStringsSep "\n" (
                    index: entry:
                      ...
                      ) "map type entry must have only 'key' and 'value' attributes."; "${indent nextIndent}${toRON' nextIndent entry.key}: ${toRON' nextIndent entry.value}${
                        optionalString (index != count) ","
                      }"
                  )
                  value.value
                },\n${indent startIndent}}"
            else if value.__type == "tuple"
            then
              assert mkAssertion "toRON" (value ? value) "tuple type must have a value.";
              assert mkAssertion "toRON" (isList value.value) "tuple type value must be a list."; let
                count = length value.value;
              in
                if count == 0
                then "()"
            ...
            then
              ...
              assert mkAssertion "toRON" (isAttrs value.value) "namedStruct type value must be a attribute set."; let
              ...
              in
                ...
                then "${value.name}()"
                  ...
                  concatImapStringsSep "\n" (
                    index: key: "${indent nextIndent}${key}: ${toRON' nextIndent value.value.${key}}${
                      optionalString (index != count) ","
                    }"
                  )
                  keys
                },\n${indent startIndent})"
            else mkThrow "toRON" "set type ${toString value.__type} is not supported."
          else let
          ...
          in
            ...
            then "()"
              ...
              concatImapStringsSep "\n" (
                index: key: "${indent nextIndent}${key}: ${toRON' nextIndent value.${key}}${
                  optionalString (index != count) ","
                }"
              )
              keys
      ...
      .${type}
      or (mkThrow "toRON" "${type} is not supported.");
  in
    toRON';
}
